    ...
    public static function createPayment($data)
    {
        ...
        if($data->transaction->bank=="card")
        {
            ...
            else
            {
                 ...
                 if($data1->failure_reason=="ISSUER_SUSPECT_FRAUD") $message = 'The card has been declined by the issuing bank due to potential fraud suspicion. Try to use USD payment method';
                 if($data1->failure_reason=="DECLINED_BY_PROCESSOR") $message = 'The card has been declined by the processor. Try to use USD payment method';
                 if($data1->failure_reason=="INSUFFICIENT_BALANCE") $message = 'The card does not have enough balance. Try to use USD payment method';
                 if($data1->failure_reason=="STOLEN_CARD") $message = 'The card has been marked as stolen. Try to use USD payment method';
                 if($data1->failure_reason=="INACTIVE_OR_UNAUTHORIZED_CARD") $message = 'The card is inactive or unauthorized to perform the transaction. Try to use USD payment method';
                 if($data1->failure_reason=="PROCESSOR_ERROR") $message = 'The charge failed because there\'s an integration issue between the card processor and the bank. Try to use USD payment method';
                 if($data1->failure_reason=="INVALID_CVV") $message = 'The card is declined due to unmatched CVV / CVC. Try to use USD payment method';
                 if($data1->failure_reason=="DECLINED_BY_ISSUER") $message = 'The card is declined by the issuing bank. Try to use USD payment method';
