...
<script type="text/javascript">
        		...
        		ok: {
            		...
            		action: function(){
                 		 var table = $('#dataTableBuilder').DataTable();
     						   ...
     						   type: 'DELETE',
     						   url: '{{ route('route_toursdk_review.index') }}/'+ id
						        }).done(function( msg ) {
	...
	function CREATE()
	{
		$.fancybox.open({
        	type: 'ajax',
       	 	src: '{{ route('route_toursdk_review.create') }}',
			touch: false,
			modal: true,
   		});	
	}
	...
	function EDIT(id)
	{
		$.fancybox.open({
        	type: 'ajax',
       	 	src: '{{ route('route_toursdk_review.index') }}/'+ id +'/edit',
			modal: true,
   		});
  ...
  function SHOW()
  {
    $.fancybox.open({
          type: 'ajax',
          src: '{{ route('route_toursdk_review.index') }}/structure',
      modal: false,
      });
